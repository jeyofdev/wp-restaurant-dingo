...
class InitCustomizer {
	...
	private static function customize_register ()
	{
		add_action("customize_register", function (WP_Customize_Manager $wp_customize) {
			...
			self::move_section($wp_customize, [
				"title_tagline" => "dingo_header",
				"custom_css" => "dingo_theme_option"
			]);

			// Change priority of default sections
			self::edit_priority_section($wp_customize, [
	...
	/**
	 * Edit the Priority of the sections which informs load order of sections
	 *
	 * @param WP_Customize_Manager $wp_customize
	 * @param array $args [sectionID => priority] example : ["static_front_page", 20, ...]
	 * 
	 * @return void
	 */
	private static function edit_priority_section (WP_Customize_Manager $wp_customize, array $args) : void
	{
		foreach ($args as $sectionID => $priority) {
			$wp_customize->get_section($sectionID)->priority = $priority;
		}
	}
}
